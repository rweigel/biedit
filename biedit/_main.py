...
# Directory containing this file — used to locate index.html and / assets
...
class HTTPRequestHandler(SimpleHTTPRequestHandler):
  ...
  def translate_path(self, path):
    ...
    if self.path.startswith("/ui/"):
      ...
      fullpath = os.path.join(options['app'], relpath)
      return fullpath
    else:
      return path
  ...
  def do_GET(self):
    ...
    if URL.path == '/' or URL.path[-1] == '!':
      ...
      if "infile" in query and query["infile"][0].endswith('.html'):
        ...
        async_run(html2pdf.convert(infile, outfile))
    ...
    if 'command' in post:
      ...
      if post['command'] == 'commit':
        ...
        if not repository['credentials']:
          ...
          cmd = "git push " + options['remote'] + "; echo Done"
      else:
        self.send_response(200)
        self.send_header('Content-type', 'text/html; charset=utf-8')
        self.end_headers()
      ...
      if 'markdown' in post:
        print("Writing " + os.path.join(options['dir'], file_base + '.md'))
        with open(file_base + '.md', 'w') as fh:
          fh.write(post['markdown'])

      if 'css' in post:
        print("Writing " + os.path.join(options['dir'], file_base + '.css'))
        with open(file_base + '.css', 'w') as fh:
          fh.write(post['css'])

      if 'latex' in post:
        print("Writing " + os.path.join(options['dir'], file_base + '.tex'))
        with open(file_base + '.tex', 'w') as fh:
          fh.write(post['latex'])

      if 'html' in post and post['html'] != '':
        print("Writing " + os.path.join(options['dir'], file_base + '.html'))
